...
module.exports = {
    ...
    async rename(id, newName) {
        if(!id) throw `you must provide an id`
        if(!newName) throw `you must provide an newName`
        if(typeof id !== 'string' || id.trim().length === 0) throw `value must be string and not empty`
        if(typeof newName !== 'string' || newName.trim().length === 0) throw `value must be string and not empty`
        
        id = id.trim()
        if(!ObjectId.isValid(id)) throw `You must provide a valid object id`

        const bandCollection = await bands();
        const updateBand = {name: newName}

        const updatedBandInfo = await bandCollection.updateOne(
            {_id:ObjectId(id)},
            {$set:updateBand}
        )

        if(updatedBandInfo.modifiedCount === 0) {
            throw `could not update band name`
        }

        const newBandName = await this.get(id);
        return newBandName
    }
